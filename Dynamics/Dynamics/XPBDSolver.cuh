...
namespace xpbd {
	...
	struct StretchProjector {
		...
		__device__ __forceinline__ void operator()(VertexDevice ver, ConstraintDevice cons, int consIndex, float tstep, int iterationCount) const {
			projectStretchConstraint(ver, cons, consIndex, tstep, iterationCount, lambdas[consIndex]);
		}
	};
	struct BendingProjector {
		float* lambdas;
		__device__ __forceinline__ void operator()(VertexDevice ver, ConstraintDevice cons, int consIndex, float tstep, int iterationCount) const {
			projectBendingConstraint(ver, cons, consIndex, tstep, iterationCount, lambdas[consIndex]);
		}
	};
}
